...
//Ejercicio 1.- Modificacion

void peluqueria::peluquerosxtipo()
{
    int nPelu = L.longitud();
    peluquero auxP;

    cout << "\t\t\tPeluqueros" << endl;

    for(int i=1 ; i<=3 ; i++) //Recorrido para cada una de las especialidades
    {
        cout << "-----------------------------------------------------------" << endl;
        cout << "Especialidad: " << i << endl;

        for(int j=1 ; j<=nPelu ; j++)
        {
            if(L.observar(j).TipoServicio == i)//Si coincide el tipo de servicio con el que estamos haciendo la busqueda
            {
                copiarpeluquero(auxP , L.observar(j)); //copiamos en auxP el peluquero que estamos leyendo en ese momento

                cout << "Peluquero: " << auxP.Codigo << "\tNombre: " << auxP.Nombre << " " << auxP.Apellidos << endl;
            }
        }
    }
    cout << "-----------------------------------------------------------" << endl;
    cout << "Total: " << nPelu << " peluqueros " << endl;
}

//Ejercicio 2.- Modificacion

void peluqueria::tiempoMedioEspera(int h , int m)
{
    int tParametro = h*60 + m; //Para pasarlo a minutos
    int tGlobal = 0;
    int tEspera ;
    int tCola = 0;
    int nPelu = L.longitud();
    ...
    cout << "\t\t\tTiempos de Espera" << endl;
    cout << "-----------------------------------------------------------" << endl;
    cout << "Peluqueros: " << endl;

    for(int i=1 ; i<=nPelu ; i++) //Recorrido por la lista de peluqueros
    {
        cout << L.observar(i).Nombre ;

        copiarpeluquero(auxP , L.observar(i)); //copio en aux lo que estoy leyendo

        nCliTot+=auxP.Col.longitud();

        while(!auxP.Col.esvacia())
        {
            tEspera=tParametro - auxP.Col.primero().HoraLlegada;
            tCola+=tEspera;
            auxP.Col.desencolar();
        }

        tCola/=auxP.Col.longitud(); //Para calcular el tiempo medio de espera en la cola

        cout << "Peluquero: " << auxP.Codigo << "\tNombre: " << auxP.Nombre << " " << auxP.Apellidos << " Especialidad: " << auxP.TipoServicio << "Tiempo de espera:" << tCola << endl;
    ...
    cout << "Tiempo medio de espera actual: " << tGlobal << " minutos " << endl;
}
