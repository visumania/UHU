...
class Taxi
{
    ...
    float KmHechos;     //Kilometros recorridos en el día
    float Ganancia;    //Ganancia del día
    float Deposito;    //Litros en el Depósito
public:
    Taxi();
...
Taxi::Taxi()
{
    strcpy(Conductor, ""); // Cadena Conductor Vacía
    Kilometros=0.0;
    KmHechos=0.0;
    Ganancia=0.0;
    Deposito=0.0;
...
void Taxi::Inicializar(cadena pConductor, float pKilometros)
{
    ...
    KmHechos=0.0;
    Ganancia=0.0;
    Deposito=80.0; // Inicializamos el despósito a 80 Litros.
}
...
bool Taxi::Carrera(float pDistancia, float &pPrecio)
{
    ...
    Consumo_Taxi = (pDistancia*Consumo_Medio)/100; // El Resultado nos dirá cuanto Combustible gastaría en el viaje.
    ...
    if(resultado)
    {
        ...
        Deposito = Deposito - Consumo_Taxi; // Deposito anterior + Deposito después del viaje
            ...
            case 1: if (NTaxi<50)
                    {
                        ...
                        cin >>Cond;
                        cout<<"Kilómetros iniciales del coche: ";
                        cin >>Kilo;
                        Flota[NTaxi].Inicializar(Cond,Kilo);
                        NTaxi++;
                        cout << "Taxi añadido correctamente.";
                    }
